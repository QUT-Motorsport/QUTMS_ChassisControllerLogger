 ...
 * SDInterface.c
 *
 * Created: 16/01/2020 11:26:34 AM
 *  Author: Calvin
 * based off http://www.rjhcoding.com/avrc-sd-interface-4.php
 */
...
void SD_EnableCS() {
    SDCARD_PORT_CS &= ~(1 << SDCARD_PIN_CS);
}
void SD_DisableCS() {
    SDCARD_PORT_CS |= (1 << SDCARD_PIN_CS);
}
...
uint8_t SD_init()
{
    ...
    while((res[0] = SD_goIdleState()) != SD_IN_IDLE_STATE)
    {
        ...
        {
            uart_putformatted("res1 result: %d\r\n", res[0]);
            uart_putformatted("NOt in idle statfe %d\r\n", 3);
            return SD_ERROR;
        }
    }
...
uint8_t SD_readRes1()
{
    ...
    while((res1 = SPI_send_byte(0xFF)) == 0xFF)
    {
        i++;
...
{
    while(n--) *res++ = SPI_send_byte(0xFF);
}
...
void SD_sendIfCond(uint8_t* res)
{
    ...
    SD_readRes7(res);
    //SD_readBytes(res + 1, R7_BYTES);
    uart_putformatted("cmd8 result: %d\r\n", res[0]);
    ...
    *token = 0xFF;
    ...
    if(res1 != 0xFF)
    {
        ...
        while(++readAttempts != SD_MAX_READ_ATTEMPTS)
            if((read = SPI_send_byte(0xFF)) != 0xFF) break;

        // if response token is 0xFE
        if(read == SD_START_TOKEN)
        {
            // read 512 byte block
            for(uint16_t i = 0; i < SD_BLOCK_LEN; i++) *buf++ = SPI_send_byte(0xFF);
...
uint8_t SD_writeSingleBlock(uint32_t addr, uint8_t* buf, uint8_t* token)
{
    ...
    *token = 0xFF;
    ...
    {
        // send start token
        SPI_send_byte(SD_START_TOKEN);

        // write buffer to card
        for(uint16_t i = 0; i < SD_BLOCK_LEN; i++) SPI_send_byte(buf[i]);

        // wait for a response (timeout = 250ms)
        readAttempts = 0;
        while(++readAttempts != SD_MAX_WRITE_ATTEMPTS)
            if((read = SPI_send_byte(0xFF)) != 0xFF) {
                *token = 0xFF;
                break;
            }
...
void SD_readOCR(uint8_t* res)
{
    ...
    SPI_send_byte(0xFF);
    SD_EnableCS();
    uint8_t tmp = SPI_send_byte(0xFF);

    if(tmp != 0xFF) while(SPI_send_byte(0xFF) != 0xFF) ;
